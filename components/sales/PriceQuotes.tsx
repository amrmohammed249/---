...
interface SmartPriceQuoteProps {
  windowId: string;
}

const SmartPriceQuote: React.FC<SmartPriceQuoteProps> = ({ windowId }) => {
    ...
    const [activeQuote, setActiveQuote] = useState<Partial<PriceQuote>>({});
    ...
    useEffect(() => {
      if (lastAddedItemId) {
        setTimeout(() => { // Timeout to allow react to render the input
    ...
    const handleFinalize = useCallback(async (print: boolean = false) => {
        if (items.length === 0) {
            showToast('لا يمكن إنشاء بيان أسعار فارغ.', 'error');
            return;
        }
        if (!customer) {
    ...
    }, [items, customer, activeQuote.date, totals, addPriceQuote, showToast, resetQuote]);
        ...
        <div className="flex flex-col h-full bg-[--bg] text-[--text] font-sans">
            <header className="flex-shrink-0 bg-[--panel] dark:bg-gray-800 shadow-sm p-3 flex flex-wrap justify-between items-center gap-4">
                ...
                <div className="flex-grow max-w-sm relative">
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="ابحث عن عميل... (Ctrl+K)"
                    ...
                    {customerSearchResults.length > 0 && (
                        <div className="absolute top-full right-0 left-0 bg-[--panel] dark:bg-gray-800 shadow-lg rounded-b-lg border dark:border-gray-700 z-30">
                            {customerSearchResults.map((c, index) => (
                                <div key={c.id} 
                                    onClick={() => { setCustomer(c); setCustomerSearchTerm(''); productSearchRef.current?.focus(); setIsDirty(true); }} 
                                    className={`p-2 cursor-pointer ${index === highlightedCustomerIndex ? 'bg-blue-100 dark:bg-gray-700' : 'hover:bg-gray-100 dark:hover:bg-gray-900'}`}
                                >
                                    {c.name} - {c.phone}
                                </div>
                            ))}
                ...
                <div className="flex items-center gap-2">
                    {customer ? (
                        ...
                        <button onClick={() => setAddCustomerModalOpen(true)} className="flex items-center gap-2 px-3 py-2 text-sm bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                            <PlusIcon className="w-4 h-4" /> عميل جديد
                        </button>
                    )}
                </div>
            </header>
            ...
            <div className="flex-1 flex flex-col md:flex-row gap-4 p-4 overflow-hidden">
                            ...
                            <div className="absolute top-full right-3 left-3 bg-[--panel] dark:bg-gray-800 shadow-lg rounded-b-lg border dark:border-gray-700 z-20">
                                {productSearchResults.map((p, index) => (
                                    <div key={p.id} 
                                     ...
                                     >
                                        <span>{p.name}</span>
                                        <span className="text-sm text-gray-500">المتاح: {p.stock}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <aside className="w-full md:w-1/3 flex flex-col gap-4">
                    <div className="bg-[--panel] dark:bg-gray-800 rounded-lg shadow-[--shadow] p-4 flex-grow flex flex-col">
                        <h2 className="text-lg font-bold border-b dark:border-gray-700 pb-2 mb-4">ملخص البيان</h2>
            ...
            {isAddCustomerModalOpen && (
                <Modal isOpen={isAddCustomerModalOpen} onClose={() => setAddCustomerModalOpen(false)} title="إضافة عميل جديد">
                    <AddCustomerForm onClose={() => setAddCustomerModalOpen(false)} onSuccess={(c) => { setCustomer(c); setAddCustomerModalOpen(false); setIsDirty(true); }} />
                </Modal>
            )}
            {quoteToView && (
...
export default SmartPriceQuote;