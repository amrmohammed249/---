...
const PurchaseQuotes: React.FC<SmartQuoteProps> = ({ windowId }) => {
    const { suppliers, inventory, addPurchaseQuote, showToast, sequences, scannedItem } = useContext(DataContext);
    const { visibleWindowId, setWindowDirty } = useContext(WindowContext);
    ...
    const [activeQuote, setActiveQuote] = useState<Partial<PurchaseQuote>>({});
    ...
    }, [items, supplier, activeQuote.date, totals, addPurchaseQuote, showToast, resetQuote]);
    ...
    const handleSupplierSearchKeyDown = (e: React.KeyboardEvent) => {
        ...
        if (e.key === 'ArrowDown') {
            e.preventDefault();
            setHighlightedSupplierIndex(prev => (prev + 1) % supplierSearchResults.length);
        } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            setHighlightedSupplierIndex(prev => (prev - 1 + supplierSearchResults.length) % supplierSearchResults.length);
        } else if ((e.key === 'Enter' || e.key === 'Tab') && highlightedSupplierIndex > -1) {
            e.preventDefault();
            setSupplier(supplierSearchResults[highlightedSupplierIndex]);
            setIsDirty(true);
            setSupplierSearchTerm('');
    ...
    const handleItemInputKeyDown = (e: React.KeyboardEvent, index: number, field: 'quantity' | 'unit' | 'price') => {
        const moveFocus = (targetIndex: number) => {
            if (targetIndex >= 0 && targetIndex < items.length) {
                ...
                if (fieldRef) {
                    fieldRef.focus();
                    if (fieldRef.tagName === 'INPUT') {
                        (fieldRef as HTMLInputElement).select();
                    }
                }
            }
        };
        ...
        <div className="flex flex-col h-full bg-[--bg] text-[--text] font-sans">
            <header className="flex-shrink-0 bg-[--panel] dark:bg-gray-800 shadow-sm p-3 flex flex-wrap justify-between items-center gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <input 
                      ...
                      onChange={e => { setActiveQuote(p => ({...p, date: e.target.value})); setIsDirty(true); }}
                ...
                <div className="flex-grow max-w-sm relative">
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="ابحث عن مورد... (Ctrl+K)"
                        value={supplierSearchTerm}
                        onChange={(e) => setSupplierSearchTerm(e.target.value)}
                        onKeyDown={handleSupplierSearchKeyDown}
                        className="w-full bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg py-2 pr-10 pl-4 focus:ring-2 focus:ring-[--accent] outline-none"
                    />
                    {supplierSearchResults.length > 0 && (
                        <div className="absolute top-full right-0 left-0 bg-[--panel] dark:bg-gray-800 shadow-lg rounded-b-lg border dark:border-gray-700 z-30">
                            {supplierSearchResults.map((c, index) => (
                                <div key={c.id} 
                                    onClick={() => { setSupplier(c); setSupplierSearchTerm(''); productSearchRef.current?.focus(); setIsDirty(true); }} 
                                    className={`p-2 cursor-pointer ${index === highlightedSupplierIndex ? 'bg-blue-100 dark:bg-gray-700' : 'hover:bg-gray-100 dark:hover:bg-gray-900'}`}
                                >
                                    {c.name} - {c.phone}
                                </div>
                            ))}
                ...
                <div className="flex items-center gap-2">
                    {supplier ? (
                        <div className="flex items-center gap-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 p-2 rounded-lg">
                            ...
                            <button onClick={() => { setSupplier(null); setIsDirty(true); }} className="text-red-500 mr-1 text-lg font-bold">×</button>
                        </div>
                    ) : (
                        <button onClick={() => setAddSupplierModalOpen(true)} className="flex items-center gap-2 px-3 py-2 text-sm bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                            <PlusIcon className="w-4 h-4" /> مورد جديد
                        </button>
                    )}
                </div>
            </header>
            ...
            <div className="flex-1 flex flex-col md:flex-row gap-4 p-4 overflow-hidden">
                <main className="w-full md:w-2/3 flex flex-col bg-[--panel] dark:bg-gray-800 rounded-lg shadow-[--shadow]">
                    <div className="p-3 border-b dark:border-gray-700 relative">
                        ...
                        <input
                            ...
                            type="text"
                            placeholder="ابحث بالباركود أو اسم الصنف... (Ctrl+F)"
                            value={productSearchTerm}
                            onChange={(e) => setProductSearchTerm(e.target.value)}
                            onKeyDown={handleProductSearchKeyDown}
                            className="w-full text-lg bg-gray-100 dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-lg p-3 pr-12 focus:ring-2 focus:ring-[--accent] outline-none"
                        />
                            ...
                            <div className="absolute top-full right-3 left-3 bg-[--panel] dark:bg-gray-800 shadow-lg rounded-b-lg border dark:border-gray-700 z-20">
                                {productSearchResults.map((p, index) => (
                                    <div key={p.id} 
                                        ...
                                        className={`p-3 flex justify-between cursor-pointer ${index === highlightedProductIndex ? 'bg-blue-100 dark:bg-gray-700' : 'hover:bg-gray-100 dark:hover:bg-gray-900'}`}
                                     >
                                        <span>{p.name}</span>
                                        <span className="text-sm text-gray-500">سعر الشراء: {p.purchasePrice}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        <table className="w-full text-sm text-right table-auto">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700/50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-2 py-3 w-28">السعر</th>
                                    <th className="px-2 py-3 w-28">الخصم</th>
                                    <th className="px-2 py-3 w-32 text-left">الإجمالي</th>
                                ...
                                {items.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={item.itemId + index} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/20">
                                            <td className="px-2 py-2 text-xs font-mono text-center">{item.itemId}</td>
                                            <td className="px-2 py-2 font-semibold truncate">{item.itemName}</td>
                                            <td className="px-2 py-2">
                                                <input 
                                                    ...
                                                    type="number" value={item.quantity} onChange={e => handleItemUpdate(index, 'quantity', e.target.value)} 
                                            ...
                                            <td className="px-2 py-2">
                                                <input 
                                                    ...
                                                    type="number" value={item.price} onChange={e => handleItemUpdate(index, 'price', e.target.value)}
                ...
                <aside className="w-full md:w-1/3 flex flex-col gap-4">
                    ...
                    <div className="bg-[--panel] dark:bg-gray-800 rounded-lg shadow-[--shadow] p-4 space-y-3">
                        <button onClick={() => handleFinalize(false)} disabled={isProcessing} className="w-full text-xl font-bold p-4 bg-[--accent-2] text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-wait">
                            {isProcessing ? 'جاري الحفظ...' : 'حفظ (F2)'}
                        </button>
                        <button onClick={() => handleFinalize(true)} disabled={isProcessing} className="w-full text-xl font-bold p-4 bg-[--accent] text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-wait">
                            {isProcessing ? 'جاري الحفظ...' : 'حفظ وطباعة (F9)'}
                        </button>
                        <button onClick={resetQuote} className="w-full text-md p-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                            طلب شراء جديد (F3)
                        </button>
                    </div>
                </aside>
            ...
            {isAddSupplierModalOpen && (
                <Modal isOpen={isAddSupplierModalOpen} onClose={() => setAddSupplierModalOpen(false)} title="إضافة مورد جديد">
                    <AddSupplierForm onClose={() => setAddSupplierModalOpen(false)} onSuccess={(c) => { setSupplier(c); setAddSupplierModalOpen(false); setIsDirty(true); }} />
                </Modal>
            )}
            {quoteToView && (
